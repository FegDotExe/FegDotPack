#TODO: general review to see how the code handles spectators
scoreboard players set @e[type=armor_stand,name="fegDsCounter"] fegDsToSwitch 0
execute as @e[type=player,scores={fegDsSwitched=2}] run scoreboard players add @e[type=armor_stand,name="fegDsCounter"] fegDsToSwitch 1
...
#execute as @e[name="fegDsPointer"] run tellraw @a {"score":{"name":"@s","objective":"fegDsSwitched"}}

scoreboard players set @e[type=player,limit=1,sort=random,scores={fegDsSwitched=2}] fegDsSwitched 3

execute as @e[type=player,scores={fegDsSwitched=3}] run scoreboard players operation @e[type=armor_stand,name="fegDsPointer"] fegDsSwitched -= @s fegDsSwId
execute as @e[type=player,scores={fegDsSwitched=3}] run teleport @s @e[type=armor_stand,name="fegDsPointer",limit=1,scores={fegDsSwitched=0}]

scoreboard players set @e[type=player,scores={fegDsSwitched=3}] fegDsSwitched 4
